...
class SRTFDynamicBatchScheduler:
    """
    Shortest Remaining Time First (SRTF) Dynamic Batch Scheduler
    
    The preemptive version of SJF Dynamic Batch scheduling.
    ...
    - Prioritizes jobs with shortest remaining time
    - Should outperform both SJF dynamic batch and SRTF batch
    ...
    def decide(self):
        """
        Decide what to do next using SRTF dynamic algorithm:
        1. If queue and GPU are empty, wait
        2. Always try to schedule shortest-remaining-time jobs (dynamic batching)
        ...
        # DYNAMIC: Always try to schedule new requests, even if GPU is not empty
        ...
        if len(scheduled_request_ids) > 0 and not self._gpu_view.is_valid_step():
            removed = self._gpu_view.request_views.pop()
            self._queue.insert(0, removed)
